...
def extract_ann_image(image, ann_poly):
    ...
    x_min, y_min = min([ann_poly[2*i] for i in range(len(ann_poly)//2)]), min([ann_poly[2*i+1] for i in range(len(ann_poly)//2)])
    x_max, y_max = max([ann_poly[2*i] for i in range(len(ann_poly)//2)]), max([ann_poly[2*i+1] for i in range(len(ann_poly)//2)])

    # Define mask
    mask = np.ones(( int(y_max)-int(y_min)+1,int(x_max)-int(x_min) +1,3), dtype=np.uint8)
    mask.fill(255)

    # Extract the image
    extracted_im = image[int(y_min):int(y_max)+1,int(x_min):int(x_max)+1,:]
...
def resize_extracted(crop, mask, key_points, r_w, r_h):
    ...
    #plt.imshow(re_mask)
    #plt.scatter([resized_annotation[2*i] for i in range(len(resized_annotation)//2)] , [resized_annotation[2*i+1] for i in range(len(resized_annotation)//2)] )
    #plt.show()
